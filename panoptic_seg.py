...
from detectron2.projects.panoptic_deeplab.DLIM_transformer import CustomTransformer, process_feature_map
...
class PanopticDeepLab(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.transformers = nn.ModuleDict({
            'res2': CustomTransformer(feature_size=256, nhead=8, nhid=512, nlayers=3),
            'res3': CustomTransformer(feature_size=512, nhead=8, nhid=1024, nlayers=3),
            'res5': CustomTransformer(feature_size=2048, nhead=8, nhid=4096, nlayers=3)
        })
    ...
    def forward(self, batched_inputs):
        ...
        #print(features.keys())
        #print("res2, res3, res5")
        #print(features['res2'].shape, features['res3'].shape, features['res5'].shape)

        processed_features = {key: process_feature_map(features[key], self.transformers[key]) for key in features}
        features = processed_features

        #print("processed_features.shape: ", processed_features.keys())
